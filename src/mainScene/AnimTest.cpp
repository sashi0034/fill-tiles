...
namespace mainScene
{
    ...
    CoroTask AnimTest::wait(coroutine<CoroTask>::push_type &yield, AnimTest *self)
    {
        ...
        mainRoot.GetTextureAnimator().TargetTo(self->m_Texture)
                ->AnimPosition(Vec2<double>{64, 64}, 3.0)->SetEase(EAnimEase::InBack)->SetLoop(2)->SetRelative(false)
                ->With()
                ->AnimRotation(60, 3.0)->SetEase(EAnimEase::Linear)->SetLoop(2)
                ->Then()
                ->AnimPosition(Vec2<double>{32, 32}, 3.0)->SetEase(EAnimEase::OutBack)->SetLoop(2)->SetRelative(true)
                ->Then()
                ->VirtualDelay([]() { std::cout << "hoge hoge"; }, 0);
